    ...
    while true do
        ...
        if IsEntityDead(playerPed) and not alreadyDead then
            ...
            if checkDeathCause (Melee, death) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." a était tabasser par "..killername)
            elseif checkDeathCause (Bullet, death) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." a était shooter par "..killername)
            elseif checkDeathCause (Knife, death) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." a était assassiner par "..killername)
            elseif checkDeathCause (Car, death) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." a était percuter par "..killername)
            elseif checkDeathCause (Animal, death) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." a était tuer par un animal.")
            elseif checkDeathCause (FallDamage, death) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." est mort suite à une chute.")
            elseif checkDeathCause (Explosion, death) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." est mort suite à une explosion.")
            elseif checkDeathCause (Gas, death) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." est mort d'asphyxie.")
            elseif checkDeathCause (Burn, death) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." est mort de brûlure.")
            elseif checkDeathCause (Drown, death) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." est mort d'asphyxie.")
            else
                TriggerServerEvent("coykn3_logs:SendLogs", "Kill", "Red", "Mort d'un joueur", ""..playerName.." est mort de raison inconnue.")
            end
        ...
        if IsPedSittingInAnyVehicle(playerPed) and not alreadyInCar then
            ...
            if isBlackListedCar(GetEntityModel(vehicle)) then
                TriggerServerEvent("coykn3_logs:SendLogs", "Car", "Blue", "Utilisation d'un véhicule", ""..playerName.." est monter dans un véhicule interdit.")
                alreadyInCar = true
            end
        end
